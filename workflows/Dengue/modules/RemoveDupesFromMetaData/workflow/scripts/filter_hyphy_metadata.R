...
# Load each package without displaying startup messages
...
opt_parser <- OptionParser(
  option_list = list(
    ...
    make_option(c("-f", "--fasta"), type = "character", help = "Input de-duped fasta file."),
...
# Load Metadata
if (!is.null(opt$csv)) {
  metadata <- read.csv(opt$csv)
} else {
  stop("Input metadata file is missing. Exiting now...")
}

metadata$name_hyphy <- gsub("\\|", "_", metadata$name)

metadata$name_hyphy <- gsub("\\.", "_", metadata$name_hyphy)

metadata$name_hyphy <- gsub("-", "_", metadata$name_hyphy)

# Clean Metadata
metadata_cleaned <- filter(metadata, name_hyphy %in% seq_names$V1)

# Write Cleaned Metadata
write.csv(metadata_cleaned,
          file = paste0(opt$outfile),
