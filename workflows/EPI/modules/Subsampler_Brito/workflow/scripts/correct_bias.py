...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--genome-matrix",
        required=True,
        help="TSV file showing the original genome counts per epiweek",
    )
    parser.add_argument(
        "--case-matrix",
        required=True,
        help="TSV file showing the case counts per epiweek",
    )
    parser.add_argument(
        "--index-column",
        required=True,
        help="Column with unique geographic information",
    )
    parser.add_argument(
        "--baseline", required=False, type=float, help="Baseline sampling proportion"
    )
    parser.add_argument(
        "--output1",
        required=True,
        help="TSV file showing genome sampling proportions per epiweek",
    )
    parser.add_argument(
        "--output2",
        required=True,
        help="TSV file showing genome sampling bias per epiweek",
    )
    parser.add_argument(
        "--output3",
        required=True,
        help="TSV file showing corrected genome counts per epiweek",
    )
    ...
    nonDateCols = [
        column for column in dfG.columns.to_list() if not column[-1].isdecimal()
    ]
    ...
    for idx, row in dfG.iterrows():
        ...
        for epiweek in date_intersection:
            ...
            if int(case_count) > 0 and int(genome_count) > 0:
                ...
                corrected_count = 0
                if (
                    case_count * global_samp_prop >= 0.9
                ):  # defines how to proceed when requested values is below 1 genome
            ...
            dfW[epiweek] = pd.to_numeric(
                dfW[epiweek], downcast="integer", errors="ignore"
            )
