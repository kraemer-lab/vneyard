...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input", required=True, help="TSV matrix of daily case counts per location"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output", required=True, help="TSV matrix of weekly case counts per location"
    )
    ...
    def epiweek_covert(df):
        ...
        df["epiweek"] = df.index.map(
            get_epiweeks
        )  # create new column 'epiweek', mapping 'dates'
        df = df.groupby(
            ["epiweek"], as_index=True
        ).sum()  # group dates from same epiweek, sum up counts
        return df.transpose()
