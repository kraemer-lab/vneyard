...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input", required=True, help="Matrix of daily counts per location"
    )
    parser.add_argument(
        "--unit",
        required=True,
    ...
    parser.add_argument(
        ...
        help="What is the format of the data points (float/integer)?",
    )
    parser.add_argument(
        "--weekasdate",
        required=False,
        ...
        help="If representing weeks as date, which day of the week should be used?",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output", required=True, help="TSV matrix with aggregated counts"
    )
    ...
    def unit_coverter(df):
        ...
        df["time_variable"] = df.index.map(
            get_newunit
        )  # create new column 'time_variable', mapping 'dates'
        df = df.groupby(
            ["time_variable"], as_index=True
        ).sum()  # group dates from same 'unit', sum up counts
        return df.transpose()
