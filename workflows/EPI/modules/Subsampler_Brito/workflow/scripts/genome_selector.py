...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="strain",
        help="Column showing genome names (e.g. USA/CT-CDC-LC0062417/2021",
    )
    parser.add_argument(
        "--accno-column",
        required=False,
        type=str,
        default="gisaid_epi_isl",
        help="Column showing genome accession numbers (e.g. EPI_ISL_2399048",
    )
    parser.add_argument("--scheme", required=True, help="Subsampling scheme")
    parser.add_argument(
        "--report", required=False, help="Report listing samples per category"
    )
    ...
    if keep == None:
        ...
        to_keep = [
            strain.strip()
            for strain in open(keep, "r").readlines()
            if strain[0] not in ["#", "\n"]
        ]
    ...
    dfN = dfN[
        dfN["date"].apply(lambda x: len(x.split("-")) == 3)
    ...
    for category in purposes:
        ...
        for idx, value1 in (
            dfS.loc[dfS["purpose"] == category, "value"].to_dict().items()
        ):
            print("\n > Filter #" + str(idx + 1))
            ...
            for epiweek, dfEpiweek in gEpiweek:
                ...
                sampled = int(
                    np.ceil((bin_pool / total_genomes) * sample_size)
                )  # proportion sampled from bin
                if (
                    sampled > bin_pool
                ):  # if requested amount is higher than available genomes, get all
                    sampled = bin_pool
                ...
                for id in results.keys():
                    ...
                    results[id][filter1][value1] = (
                        results[id][filter1][value1] + selected
                    )
                    if id == accno_col:
    ...
    if len(not_found) > 0:
        print(
            ...
            + " out of "
            + str(len(to_keep))
            + " pre-selected genomes(s) were not found:"
        )
    ...
    print(
        "\n"
        + str(genome_count)
        + " genome(s) exported according to subsampling scheme\n"
    )
