...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input", required=False, help="Johns Hopkins, global daily case counts file"
    )
    parser.add_argument(
        "--download",
        required=True,
        ...
        default="yes",
        choices=["yes", "no"],
        help="Download global case series from Johns Hopkins University?",
    )
    parser.add_argument(
    ...
    if download == "yes":
        ...
        df = pd.read_csv(
            "time_series_covid19_global.csv", encoding="utf-8", sep=",", dtype="str"
        )
    else:
        input = args.input
    ...
    for num, col in enumerate(date_columns):
        ...
        if num < len(date_columns) - 1:
            ...
            for idx, row_value in df[col].items():
                ...
                print(
                    country, date_columns[num], daily_count
                )  # , int(row_value), int(df[date_columns[num+1]][idx]))

    # save processed metadata
    df.to_csv("time_series_covid19_global_reformatted.tsv", sep="\t", index=False)
    print(
        "\nOutput successfully exported: "
        + "time_series_covid19_global_reformatted.tsv\n"
    )
