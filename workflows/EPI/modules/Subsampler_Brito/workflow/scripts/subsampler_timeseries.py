...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sequences",
        required=False,
        help="FASTA file with genomes named as in metadata",
    )
    parser.add_argument("--metadata", required=True, help="Metadata TSV file")
    parser.add_argument(
        "--genome-matrix",
        required=True,
        help="TSV file showing corrected genome counts per unit of time",
    )
    parser.add_argument(
        "--max-missing",
        required=False,
        type=int,
        default=99,
        help="Maximum percentage of Ns or gaps (int = 1-100)",
    )
    parser.add_argument(
        "--refgenome-size",
        required=False,
        type=int,
        default=1,
        help="Reference genome size",
    )
    parser.add_argument(
        "--keep", required=False, help="List of samples to keep, in all instances"
    )
    parser.add_argument(
        "--remove", required=False, help="List of samples to remove, in all instances"
    )
    parser.add_argument(
        "--filter-file",
        required=False,
        help="TSV file listing columns/values of samples to be batch included or excluded",
    )
    parser.add_argument(
        "--seed",
        required=False,
        type=int,
        help="Seed number for pseudorandom sampling of genomes",
    )
    parser.add_argument(
        "--index-column",
        required=True,
        help="Metadata column with unique genome identifiers (genome names, accession codes, etc",
    )
    parser.add_argument(
        "--geo-column",
        required=True,
        help="Metadata column with the target geographic information (country, division, etc)",
    )
    parser.add_argument(
        "--date-column",
        required=True,
    ...
    parser.add_argument(
        ...
        type=str,
        default="week",
        choices=["week", "month", "year", "full"],
        help="Time unit for conversion",
    )
    parser.add_argument(
        "--weekasdate",
        required=False,
        ...
        type=str,
        default="no",
        choices=["start", "end", "no"],
        help="When representing weeks as date, which day of the week should be used?",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--report",
        required=True,
        help="List of statistics related to the sampling scheme",
    )
    ...
    def load_table(file):
        ...
        if str(file).split(".")[-1] == "tsv":
            ...
            df = pd.read_excel(
                file, index_col=None, header=0, sheet_name=0, dtype="str"
            )
    ...
    if fasta_file not in ["", None]:
        ...
        for fasta in SeqIO.parse(open(fasta_file), "fasta"):
            ...
            if size > min_size:
                fasta_headers = pd.concat([fasta_headers, id])
            else:
                print(
                    "size: "
                    + str(size)
    ...
    def filter_df(df, criteria):
        ...
        for filter_value in criteria.split(","):
            ...
            if not filter_value.startswith("~"):
                ...
                if col not in include:
                    ...
                    include[col] = pd.concat([include[col], val])
        # print('Include:', include)
        for filter_col, filter_val in include.items():
            print(
                "\t- Including only rows with '"
                + filter_col
                + "' = '"
                + ", ".join(filter_val)
                + "'"
            ...
            if new_df.empty:
                ...
                new_df = new_df = pd.concat([new_df, df_filtered])
            else:
        ...
        for filter_col, filter_val in exclude.items():
            ...
            if new_df.empty:
                ...
                new_df = new_df = pd.concat([new_df, df])
            else:
                new_df = new_df[~new_df[filter_col].isin(filter_val)]
            # print(new_df)#.head())
        return new_df
    ...
    if filter_file not in ["", None]:
        ...
        dfC["action"] = dfC["action"].apply(
            lambda x: "~" if x == "exclude" else ""
        )  # exclude -XX-XX missing dates
        dfC["filter"] = (
            dfC["action"].astype(str)
            + dfC["column"].astype(str)
            + ":"
            + dfC["value"].astype(str)
        )
    ...
    dfM = dfM[
        dfM[date_col].apply(lambda x: len(x.split("-")) == 3)
    ]  # accept only full dates
    dfM = dfM[
        dfM[date_col].apply(lambda x: "X" not in x)
    ]  # exclude -XX-XX missing dates
    ...
    def filter_bydate(df, date):
        ...
        mask = (df[date] > start_date) & (
            df[date] <= end_date
        )  # mask any lines with dates outside the start/end dates
    ...
    for id in open(keep, "r").readlines():
        if id[0] not in ["#", "\n"]:
            id = id.strip()
            if id in meta_seqs:
                keep_sequences = pd.concat([keep_sequences, id])
            else:
                remove_sequences.append(id)
    ...
    def get_newunit(value):
        if value[0].isdecimal():
            ...
            if unit == "week":
                ...
                #     time_cols = pd.concat([time_cols, epiweek])
                return epiweek
            elif unit == "month":
                year_month = date.strftime("%Y-%m")
                # if year_month not in time_cols:
                # time_cols = pd.concat([time_cols, year_month])
                return year_month
            elif unit == "year":
                year = date.strftime("%Y")
                # if year not in time_cols:
                # time_cols = pd.concat([time_cols, year])
                return year
            elif unit == "full":
                return "total"
        else:
    ...
    def get_iso(country):
        ...
        if country not in codes.keys():
            try:
                isoCode = pyCountry.country_name_to_country_alpha3(
    ...
    if "code" not in dfM.columns.to_list():
        ...
        if "division" in geo_level:
            dfM["code"] = dfM[geo_level].apply(
                lambda x: us_state_abbrev[x] if x in us_state_abbrev else ""
            )
    ...
    for name, dfLevel in glevel:
        if name in dfS[geo_level].to_list():
            ...
            for time_unit, dfUnit in gUnit:
                ...
                existing = dfG.loc[
                    name, time_unit
                ]  # pre-selected sequences, if any was listed in keep.txt

                if (
                    target_sampling >= available_samples
                ):  # if requested sample number is higher than available genomes, get all
                    sampled = [
                        sample
                        for sample in dfUnit[id_col].to_list()
    ...
    outfile3.write(
        "\n\n# A total of "
        + str(len(remove_sequences))
        + " were removed due to lack of metadata, or as listed in remove.txt\n\n"
    )
    ...
    outfile3.write(
        "\n\n# A total of "
        + str(len(keep_sequences))
        + " samples were forcibly added as listed in keep.txt\n\n"
    )
