...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--metadata1", required=True, help="FASTA file with pre-existing sequences"
    )
    parser.add_argument(
        "--metadata2", required=True, help="FASTA file with new sequences"
    )
    parser.add_argument(
        "--how",
        required=False,
    ...
    if len(df1["strain"].to_list()) > 0:
        print(
            "\n\t- A total of "
            + str(len(df1["strain"].to_list()))
            + " unique entries were found in the original file."
        )

    if len(df2["strain"]) > 0:
        print(
            "\t- A total of "
            + str(len(df2["strain"].to_list()))
