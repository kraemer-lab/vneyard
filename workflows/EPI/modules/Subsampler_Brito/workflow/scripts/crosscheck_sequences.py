...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", required=True, help="FASTA file with pre-existing sequences"
    )
    parser.add_argument(
        "--new-genomes", required=True, help="FASTA file with new sequences"
    )
    parser.add_argument(
        "--max-missing",
        required=False,
    ...
    preexist_simple = list(
        map(lambda strain: str(strain).replace("-", "").replace("_", ""), preexisting)
    )
            ...
            if strain not in preexisting:
                if strain not in already_found and strain not in new_entries:
                    ...
                    print(
                        "\t- "
                        + strain
                        + ": this sequence was already downloaded. Skipping..."
                    )
    ...
    if len(already_found) > 0:
        ...
        print(
            "\nA total of "
            + str(len(already_found))
            + " were not exported (to avoid duplicates).\n"
        )
    ...
    if len(too_small) > 0:
        ...
        print(
            "\nA total of "
            + str(len(too_small))
            + " partial genomes were detected and ignored (coverage < "
            + str(100 - max_gaps)
            + "%).\n"
        )
    ...
    if len(duplicates) > 0:
        ...
        print(
            "\nA total of "
            + str(len(duplicates))
            + " duplicates were detected and ignored.\n"
        )
